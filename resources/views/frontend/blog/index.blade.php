                            ...
                            <div class="col-md-6">
                                <div class="card-body p-4">
                                    <div class="d-flex align-items-center mb-2">
                                        <span class="badge custom-btn  me-2">{{ $featuredPost->category->name }}</span>
                                        <small class="text-muted">{{ $featuredPost->created_at->format('M d, Y') }}</small>
                                    </div>
                                    <h2 class="card-title h3 mb-3" style="clear: #00b22d">{{ $featuredPost->title }}</h2>
                                    <p class="card-text text-muted">{{ Str::limit($featuredPost->excerpt, 150) }}</p>
                                    <a href="{{ route('blog.show', $featuredPost->slug) }}" class="btn custom-btn">Read
                                        More</a>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-12">
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('blog.index') }}"
                        class="btn custom-btn-outline {{ request()->routeIs('blog.index') && !request()->has('category') ? 'active' : '' }}"
                        >
                        All Posts
                    </a>
                    ...
                    @foreach ($categories as $category)
                        <a href="{{ route('blog.index', ['category' => $category->slug]) }}"
                            class="btn custom-btn-outline {{ request('category') === $category->slug ? 'active' : '' }}">
                            {{ $category->name }}
                        </a>
                    @endforeach
                </div>
            ...
            @forelse($posts as $post)
                <div class="col-md-6 col-lg-4">
                    <article class="card h-100 border-0 shadow-sm">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center mb-2">
                                <span class="badge custom-btn me-2">{{ $post->category->name }}</span>
                        ...
                        <div class="card-footer bg-transparent border-0">
                            <a href="{{ route('blog.show', $post->slug) }}" class="btn custom-btn btn-sm">Read
                                More</a>
                        </div>
                    </article>
                </div>
            @empty
                <div class="col-12">
                    <div class="text-center py-5">
                        <i class="bi bi-journal-text display-1" style="color: #00b22d;"></i>
