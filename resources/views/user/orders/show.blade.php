            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('user.orders.index') }}" class="btn custom-btn-outline">
                            <i class="fas fa-arrow-left me-2"></i>Back to Orders
                        </a>
                        <a href="{{ route('checkout.download-invoice', $order->id) }}" class="btn custom-btn-outline">
                            <i class="bi bi-download"></i> Download Invoice
                        </a>
                        @if (in_array($order->payment_status, ['pending', 'failed']))
                            <a href="{{ route('checkout.repay', $order) }}" class="btn custom-btn-outline">
                        ...
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <div class="d-flex align-items-center">
                                    <span
                                        class="badge bg-{{ $order->status_color }} fs-6 me-3">{{ ucfirst($order->status) }}</span>
                                    <div>
                                ...
                                <div class="flex-shrink-0 me-3">
                                    @if ($line->product && $line->product->image)
                                        <img src="{{ asset('storage/' . $line->product->image) }}"
                                            alt="{{ $line->product->name }}" class="rounded"
                                            style="width: 80px; height: 80px; object-fit: cover;">
                                    @else
                                        <div class="bg-light rounded d-flex align-items-center justify-content-center"
                                            style="width: 80px; height: 80px;">
                                            <i class="fas fa-box fs-4" style="color: var(--primary-color)"></i>
                                        </div>
                                    @endif
                                </div>
                                <div class="flex-grow-1">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <span class="text-muted ms-3">Price:
                                                £{{ number_format($line->price, 2) }}</span>
                                        </div>
                                        <div class="fw-semibold">£{{ number_format($line->price * $line->quantity, 2) }}
                                        </div>
                                    </div>
                                    <div class="mt-2"><a href="{{ route('user.products.download', $line->product_id) }}"
                                            class="btn custom-btn btn-sm">Download</a></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        <div class="card border-0 shadow-sm h-100">
                            ...
                            <div class="card-body">
                                @if ($order->billing_address)
                                    ...
                                    <p class="mb-1">{{ $order->billing_address['city'] ?? '' }},
                                        {{ $order->billing_address['state'] ?? '' }}
                                        {{ $order->billing_address['zip'] ?? '' }}</p>
                                    <p class="mb-1">
                                        {{ App\Models\Country::where('code', $order->billing_address['country'])->first()->name ?? '' }}
                                    </p>
                        ...
                        <ul class="timeline list-unstyled mb-0">
                            @foreach ($order->histories()->latest()->get() as $history)
                                <li class="timeline-item position-relative mb-4 pb-2">
                                    <span
                                        class="timeline-marker position-absolute top-0 start-0 translate-middle  rounded-circle"
                                        style="width:14px; height:14px; background-color: var(--primary-color);"></span>
                                    <div class="ms-4">
                                        <div class="d-flex align-items-center mb-1">
                                            ...
                                            <i class="{{ $icon }} me-2"></i>
                                            <span
                                                class="fw-semibold">{{ ucfirst(str_replace('_', ' ', $history->event)) }}</span>
                                            <span
                                                class="text-muted small ms-2">{{ $history->created_at->format('M d, Y g:i A') }}</span>
                                        </div>
                                        <div class="text-muted small">{{ $history->description }}</div>
                                        @if ($history->event === 'status_changed')
                                            <div class="small mt-1">
                                                <span class="badge bg-light text-dark">From:
                                                    <strong>{{ $history->old_value['status'] ?? '' }}</strong></span>
                                                <span class="badge bg-primary ms-2">To:
                                                    <strong>{{ $history->new_value['status'] ?? '' }}</strong></span>
                                            </div>
                                        @elseif($history->event === 'shipping_method_changed')
                                            <div class="small mt-1">
                                                <span class="badge bg-light text-dark">From:
                                                    <strong>{{ $history->old_value['shipping_method'] ?? '' }}</strong></span>
                                                <span class="badge bg-warning ms-2">To:
                                                    <strong>{{ $history->new_value['shipping_method'] ?? '' }}</strong></span>
                                            </div>
                                        @endif
                                    </div>
                                </li>
